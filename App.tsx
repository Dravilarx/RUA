...
import { initialStudents, initialTeachers, initialSubjects, initialGrades, initialActivityLog, initialAnotaciones, initialCalendarEvents, initialNewsArticles, initialGradeReports, initialOfficialDocuments, initialMeetingRecords, initialProfessionalActivities, initialTeacherProfessionalActivities, initialPersonalDocuments, initialSiteLog, initialQuickLinks, initialSurveys, surveyQuestions } from './data';
import type { Student, Teacher, Subject, Grade, ActivityLog, Anotacion, CalendarEvent, NewsArticle, GradeReport, OfficialDocument, MeetingRecord, ProfessionalActivity, TeacherProfessionalActivity, PersonalDocument, ActivityType, TeacherActivityType, SiteLog, QuickLink, Survey, SurveyAnswer } from './types';
...
const SiteManagementPage: React.FC<{ siteLog: SiteLog[], activityLog: ActivityLog[] }> = ({ siteLog, activityLog }) => (
    <div className="space-y-8">
        <div>
            <PageTitle title="Gestión del Sitio (Log de Acciones)" />
            <Card>
                <table className="w-full text-left">
                    <thead>
                        <tr className="border-b">
                            <th className="p-4 font-semibold text-dark-text">Fecha y Hora</th>
                            <th className="p-4 font-semibold text-dark-text">Usuario</th>
                            <th className="p-4 font-semibold text-dark-text">Acción</th>
                            <th className="p-4 font-semibold text-dark-text">Descripción</th>
                        </tr>
                    </thead>
                    <tbody>
                        {[...siteLog].reverse().map(log => (
                            <tr key={log.id} className="border-b last:border-0 hover:bg-slate-50">
                                <td className="p-4 text-dark-text">{log.timestamp.toLocaleString('es-CL')}</td>
                                <td className="p-4 text-dark-text">{log.user}</td>
                                <td className="p-4"><span className={`px-2 py-1 text-xs font-semibold rounded-full ${log.action.includes('Crear') ? 'bg-green-100 text-green-800' : log.action.includes('Eliminar') ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>{log.action}</span></td>
                                <td className="p-4 text-dark-text">{log.description}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
...
const PersonProfile: React.FC<any> = ({ person, anotaciones, activities, documents, gradeReports, surveys, subjects, openModal, personType }) => {
    ...
    const FullReportList = ({ reports, subjects, openModal, person }: any) => ( <ul className="space-y-3">{reports.map((r: GradeReport) => { const subject = subjects.find((s: Subject) => s.id === r.subjectId); return (<li key={r.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg"><div className="font-medium">{subject?.name} - {r.generationDate.toLocaleDateString('es-CL')}</div><div><span className={`px-2 py-1 text-xs font-semibold rounded-full mr-4 ${r.status === 'Completado' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>{r.status}</span><Button onClick={() => openModal({ type: 'VIEW_REPORT', data: { report: r, student: person, subject } })} className="bg-white border border-primary text-primary hover:bg-primary-light">Ver y Firmar</Button></div></li>)})}</ul> );
...
const App: React.FC = () => {
    ...
    const logAction = (action: string, description: string) => {
        const user = "Dr. Pérez"; // Placeholder for logged in user
    ...
    const dataContext = { students, teachers, subjects, grades, activityLog, anotaciones, calendarEvents, news, gradeReports, officialDocuments, meetingRecords, professionalActivities, teacherProfessionalActivities, personalDocuments, siteLog, quickLinks, surveys, openModal: setModal, setCurrentView };
